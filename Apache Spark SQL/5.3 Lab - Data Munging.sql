...
cast(from_unixtime(event_previous_timestamp/1e6) as timestamp) AS previousEventDate,
cast(from_unixtime(event_timestamp/1e6) as timestamp) AS EventDate,
geo.city as city,
geo.state as state,
user_id as userId
FROM eventsRaw
...
SELECT
*
FROM tempData ORDER BY purchases DESC
