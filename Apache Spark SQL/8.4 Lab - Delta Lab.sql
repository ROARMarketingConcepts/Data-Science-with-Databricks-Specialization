...
-- MAGIC * Use optional fields to indicate the path you're reading from and express that the schema should be inferred. 
...
CREATE OR REPLACE TABLE health_tracker_data_2020_delta
USING delta
PARTITIONED BY (p_device_id)
LOCATION "/health_tracker_data_2020/silver" AS (
SELECT
  value.heartrate,
  value.name,
...
SELECT * 
FROM health_tracker_data_2020_delta
LIMIT 5
...
SELECT p_device_id, count(*) as cnt
FROM health_tracker_data_2020_delta
...
AS (
  SELECT 
  COUNT(*) AS broken_readings_count, dte FROM health_tracker_data_2020_delta
  WHERE heartrate < 0
  GROUP BY dte
  ORDER BY dte
);
...
SELECT count(*) AS cnt
FROM health_tracker_data_2020_late
